...
-(instancetype)initWithFrame:(CGRect)frame withTitle:(NSArray *)titles{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        UIImageView *tagImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"notification"]];
        tagImageView.frame = CGRectMake(15, (self.frame.size.height - 15 )/2, 15, 15);
...
//        tagLabel.layer.borderWidth = 1.0f;
        ...
        self.index = 1;
        self.firstBtn = [self btnframe:CGRectMake(25, 0, [UIScreen mainScreen].bounds.size.width - 2 *30, self.bounds.size.height)  titleColor:_titleColor action:@selector(clickBtn:)];
...
-(void)nextAd{
    ...
    self.moreBtn = [self btnframe: CGRectMake(25, self.bounds.size.height,[UIScreen mainScreen].bounds.size.width -2 *30, self.bounds.size.height)  titleColor:_titleColor action:@selector(clickBtn:)];
